...
// --- NUEVA FUNCIÓN ON-DEMAND ---
export const fetchLastEvidenceForDetail = async (detailId: number) => {
    console.log("Buscando evidencia previa para el detalle ID:", detailId);
    const { data, error } = await supabase
      .from('Registros')
      .select(`
        *,
    ...
    if (error) {
      console.warn("Error buscando evidencia previa:", error.message);
      return null;
    }
